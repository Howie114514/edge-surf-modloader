...
// src/wrapper/script.ts
var import_localforage = __toESM(require_localforage());
var sml;
((sml2) => {
  sml2.eventHub = new eventemitter3_default();
})(sml || (sml = {}));
window.install = (script) => {
  if (!script.src) throw new Error("No source.");
  import_localforage.default.setItem(script.uuid, script.src);
  delete script.src;
  let scripts = JSON.parse(localStorage["scripts"] ?? "[]");
  scripts.push(script);
  localStorage["scripts"] = JSON.stringify(scripts);
};
window.uninstall = (name) => {
  let scripts = JSON.parse(localStorage["scripts"] ?? "[]");
  scripts = scripts.filter((s) => s.uuid != name);
  import_localforage.default.removeItem(name);
  localStorage["scripts"] = JSON.stringify(scripts);
};
window.sml = sml;

// src/wrapper/surfHandler.ts
var defaultStats = '{"settings":{"playerBodyColor":5,"playerHairStyle":1,"playerHairColor":5,"playerOutfitStyle":0,"playerOutfitColor":2,"playerExtraStyle":1,"gameSpeed":1,"theme":"horizon","hitbox":false,"reducedMotion":false},"highScore":{"endless":0,"timetrial":0,"zigzag":0,"collector":0}}';
var surfHandler;
((surfHandler2) => {
  function saveGameStats(stats) {
    localStorage["game-stats"] = stats;
  }
  surfHandler2.saveGameStats = saveGameStats;
  ...
  surfHandler2.getGameStats = getGameStats;
})(surfHandler || (surfHandler = {}));

// src/wrapper/handlers.ts
window.surf = {};
...
function log(...args) {
  console.log(...args);
  let i = document.getElementById("untrusted-iframe");
...
async function preload() {
  ...
  try {
    ...
    let MessageTypes;
    ((MessageTypes2) => {
      MessageTypes2["REQUEST_DATA"] = "requestData";
      MessageTypes2["RESIZE_IFRAME"] = "resizeIframe";
      MessageTypes2["SAVE_GAME_DATA"] = "saveGameData";
      MessageTypes2["GET_GAME_DATA"] = "getGameData";
      MessageTypes2["RECORD_ACTIONS"] = "recordActions";
      MessageTypes2["RECORD_THEME"] = "recordTheme";
      MessageTypes2["RECORD_GAME_MODE"] = "recordGameMode";
      MessageTypes2["RECORD_BODY_COLOR"] = "recordBodyColor";
      MessageTypes2["RECORD_OUTFIT_COLOR"] = "recordOutfitColor";
      MessageTypes2["RECORD_OUTFITS"] = "recordOutfits";
      MessageTypes2["RECORD_HAIR_COLOR"] = "recordHairColor";
      MessageTypes2["RECORD_HAIR_STYLE"] = "recordHairStyle";
      MessageTypes2["RECORD_ACCESSORY"] = "recordAccessory";
      MessageTypes2["OPEN_SIDEBAR_APP"] = "openSidebarApp";
    })(MessageTypes || (MessageTypes = {}));
    window.addEventListener("message", (ev) => {
      let data = ev.data;
      let src = ev.source;
      let untrustedOrigin = src.location.origin;
      if (ev.data.type === "resizeIframe" /* RESIZE_IFRAME */) {
        const desiredHeight = ev.data.height && ev.data.height < 580 ? ev.data.height : 580;
      ...
      switch (data.type) {
        ...
        case "saveGameData" /* SAVE_GAME_DATA */:
          ...
          break;
        case "getGameData" /* GET_GAME_DATA */:
    ...
    if (!localStorage.safeMode) {
      ...
      for (let script of scripts) {
        let src = await import_localforage2.default.getItem(script.uuid);
        if (!src) {
          log(
    ...
    log("Launching game...");
    document.getElementById("untrusted-iframe").src = "./untrusted.html";
  } catch (e) {
    log("Failed!\n", e.stack);
    log(
